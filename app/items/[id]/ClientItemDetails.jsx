...
export default function ClientItemDetails({ id }) {
  ...
  useEffect(() => {
    ...
    const foundItem = allItems.find(i => String(i.id) === String(id));
    if (foundItem) {
      setItem(foundItem);
      // Get related items (same category, excluding current)
      const related = allItems
        .filter(i => i.category === foundItem.category && String(i.id) !== String(id))
        .slice(0, 3);
